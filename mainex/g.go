...
func main() {
	taskqueue.Task.NewTask(10)
	taskqueue.Task.Soldiers(5, list)
	//listUrl(0)
}
...
func listUrl(page int) {
	...
	url := "http://www.dianping.com/search/keyword/1/10_光大会展中心/p" + pagestr
	html := GetHtml(url)
	//save(html, "28.html")
...
func savelist(goodsname, location, price, url, imageurl string, page int) {
	...
	db, err := sql.Open("mysql", "root:TYwy2016720@tcp(139.196.16.67:3306)/whateat?charset=utf8")
	if err != nil {
